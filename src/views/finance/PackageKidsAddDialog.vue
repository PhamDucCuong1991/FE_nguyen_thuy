<template>
  <el-dialog
    :title="'Thêm khoản cho học sinh tháng ' + monthText"
  ...
  >
    ...
    >
      ...
      <el-row :gutter="20">
        <el-col :span="8">
          <el-form-item label="Đăng ký" prop="active">
      ...
      <el-row v-if="dataInput.type == 'multiple'" :gutter="20">
        <el-col :span="24">
          <el-form-item label="Tháng áp dụng">
            <el-checkbox v-model="dataInput.t1" disabled label="T1"></el-checkbox>
            <el-checkbox v-model="dataInput.t2" disabled label="T2"></el-checkbox>
            <el-checkbox v-model="dataInput.t3" disabled label="T3"></el-checkbox>
            <el-checkbox v-model="dataInput.t4" disabled label="T4"></el-checkbox>
            <el-checkbox v-model="dataInput.t5" disabled label="T5"></el-checkbox>
            <el-checkbox v-model="dataInput.t6" disabled label="T6"></el-checkbox>
            <el-checkbox v-model="dataInput.t7" disabled label="T7"></el-checkbox>
            <el-checkbox v-model="dataInput.t8" disabled label="T8"></el-checkbox>
            <el-checkbox v-model="dataInput.t9" disabled label="T9"></el-checkbox>
            <el-checkbox v-model="dataInput.t10" disabled label="T10"></el-checkbox>
            <el-checkbox v-model="dataInput.t11" disabled label="T11"></el-checkbox>
            <el-checkbox v-model="dataInput.t12" disabled label="T12"></el-checkbox>
          </el-form-item>
        </el-col>
      </el-row>
      ...
      <el-row :gutter="20" v-if="dataInput.attendance && dataInput.type == 'multiple'">
...
export default {
  ...
  watch: {
    dataInput: {
      handler: function () {
        ...
        if (dataInputWatch.startDate != "" && dataInputWatch.endDate != "") {
          if (dataInputWatch.startDate > dataInputWatch.endDate) {
            this.$alert("Ngày kết thúc không thể nhỏ hơn ngày bắt đầu", "Thông báo!", {
              confirmButtonText: "Đã hiểu",
              callback: () => {
                this.dataInput.endDate = "";
              },
            });
          }
        }
    ...
    getStartDate() {
      ...
      if (dataInputWatch.startDate != "" && dataInputWatch.endDate != "") {
        if (dataInputWatch.startDate > dataInputWatch.endDate) {
          this.$alert("Ngày bắt đầu không thể lớn hơn ngày kết thúc", "Thông báo!", {
            confirmButtonText: "Đã hiểu",
            callback: () => {
              this.dataInput.startDate = "";
            },
          });
        }
      }
  ...
  computed: {
    ...
    getStartDate() {
      return this.dataInput.startDate;
    },
    getEndDate() {
      return this.dataInput.endDate;
    },
  },
  methods: {
    //reset when click x
    ...
    submitForm() {
      ...
      this.dataInput.date = this.date;
      this.$refs["dataInput"].validate((valid) => {
        if (valid) {
          this.loadingButton = true;
          FnFeesService.createPackageKidsForAdd(this.idKid, this.dataInput)
            .then((resp) => {
              this.$message({
    ...
    checkBeforeSubmit() {
      if (this.dataInput.idPackage == "") {
        this.$message({
          message: "Chưa chọn khoản",
          type: "error",
        });
        throw null;
      }
    },
    getDataInitialAddPackageKids(idKid, date) {
      this.idKid = idKid;
      this.date = date;
      this.monthText = this.moment(date).format("MM-YYYY");
      FnFeesService.getPackageForAdd(idKid, date)
        .then((resp) => {
