<template>
  <el-dialog
    title="Thêm giảm giá các khoản mặc định"
    ...
    submitForm() {
      this.$refs["dataInput"].validate((valid) => {
        if (valid) {
          ...
          this.dataInput.idKidList = this.$parent.multipleSelectionDefault.map(
            (x) => x.id
          );
          this.loadingButton = true;
          FnFeesService.addDefaultDiscountService(this.dataInput)
            .then((resp) => {
              this.$message({
